...
def plan(*rows, write_policy: Any = schema_mod.WRITE_FILL_ONLY):
...
class PlannedReplayTests(unittest.TestCase):
    ...
    def test_unresolved_v3_provider_diagnostics_are_exact(self):
        plans_mod.validate_unresolved(self.provider_unresolved())
        plans_mod.validate_unresolved(self.provider_unresolved("lastfm", "circuit_open"))
        ordinary = {"schema_version": 3, "unresolved": [{
            "post_id": 1, "post_title": "Album", "diagnostics": [{
                "code": "spotify_no_results", "message": "No match.",
            }],
        }]}
        plans_mod.validate_unresolved(ordinary)
        ignored = {"schema_version": 3, "ignored": [{
            "post_id": 1, "post_title": "Album", "diagnostics": [{
                "code": "spotify_catalog_unavailable", "message": "Unavailable.",
            }],
        }]}
        plans_mod.validate_ignored(ignored)
        ...
        for artifact in invalid:
            with self.subTest(artifact=artifact), self.assertRaises(ValueError):
                plans_mod.validate_unresolved(artifact)
    ...
    def test_v2_accepts_only_shaped_lastfm_recovery_diagnostics(self):
        for code in ("lastfm_lookup_fallback", "lastfm_transliteration_alignment"):
            with self.subTest(code=code):
                ...
                self.assertEqual(plans_mod.validate_plan(plan(row))["schema_version"], 2)
                row["diagnostics"][0]["route"] = "provider payload"
                with self.assertRaises(ValueError):
                    plans_mod.validate_plan(plan(row))

    def test_v2_write_policies_and_v1_regeneration_error(self):
        plans_mod.validate_plan(plan(write_policy=schema_mod.WRITE_FILL_ONLY))
        plans_mod.validate_plan(plan(write_policy=schema_mod.WRITE_OVERWRITE_MANAGED))
        for invalid in ("overwrite", "", None):
            with self.subTest(invalid=invalid), self.assertRaisesRegex(ValueError, "write_policy"):
                plans_mod.validate_plan(plan(write_policy=invalid))
        missing = plan(); del missing["write_policy"]
        with self.assertRaisesRegex(ValueError, "root"):
            plans_mod.validate_plan(missing)
    ...
    def test_v2_rejects_obsolete_lastfm_field_before_wordpress_construction(self):
        ...
        with self.assertRaisesRegex(ValueError, "lastfm_release_id"):
            plans_mod.validate_plan(obsolete)
    ...
    def test_strict_validation_unknown_bool_nonfinite_duplicate_and_out_of_slice(self):
        ...
        for value in mutations:
            with self.subTest(value=value), self.assertRaises(ValueError): plans_mod.validate_plan(value)
        with self.assertRaises(ValueError):
            plans_mod.validate_plan(plan(patch_row(1), {"bad": "outside slice"}))
        for offset, limit in ((-1, None), (0, -1)):
            with self.assertRaises(ValueError): plans_mod.slice_items([], offset, limit)
    ...
    def test_replay_dates_require_calendar_valid_dmy_values(self):
        ...
        self.assertEqual(plans_mod.materialize_body(write, {}), {"acf": {
    ...
    def test_tracks_and_materialization_preserve_absent_replacements(self):
        ...
        plans_mod.validate_plan(plan(row))
        self.assertEqual(plans_mod.materialize_body(row["write"], {}), row["write"])
        self.assertNotIn("categories", plans_mod.materialize_body(row["write"], {}))
    ...
    def test_resolution_once_before_updates_and_mixed_result(self):
        wp = FakeWP()
        succeeded, failed = cli_mod.apply_patches(wp, [patch_row(1), patch_row(2)])
    ...
    def test_term_resolution_failure_prevents_all_updates(self):
        ...
        with self.assertRaises(RuntimeError): cli_mod.apply_patches(wp, [patch_row()])
        self.assertEqual(wp.updated, [])

    def test_apply_plan_isolated_and_atomic_result(self):
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "planned.json"
            path.write_text(json.dumps(plan(write_policy=schema_mod.WRITE_OVERWRITE_MANAGED)))
            wp = FakeWP()
            with patch.object(cli_mod, "WordPress", return_value=wp), \
                 patch.object(cli_mod, "Spotify", side_effect=AssertionError), \
                 patch.object(cli_mod, "LastFM", side_effect=AssertionError), \
                 patch.object(cli_mod, "is_field_present", side_effect=AssertionError):
                rc = cli_mod.cmd_apply_plan(Namespace(plan=str(path), offset=0, limit=None, out_dir=td), {
    ...
    def test_command_env_requirements_and_loader_does_not_warn(self):
        with self.assertRaisesRegex(SystemExit, "SPOTIFY_CLIENT_ID"):
            cli_mod.require_env({}, "SPOTIFY_CLIENT_ID")
        with patch.object(cli_mod.log, "warning") as warning:
            cli_mod.load_env(None)
        warning.assert_not_called()

    def test_taxonomy_pagination_headers_fallback_and_errors(self):
        wp = object.__new__(wordpress_mod.WordPress); wp._url = lambda *a, **k: "url"
    ...
    def test_run_commands_write_artifacts_and_share_replay_materialization(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            dry_wp = CommandWP()
            with patch.object(cli_mod, "WordPress", return_value=dry_wp), \
                 patch.object(cli_mod, "Spotify"), patch.object(cli_mod, "LastFM"), \
                 patch.object(cli_mod, "enrich", return_value=patch_row()):
                self.assertEqual(cli_mod.cmd_run(args, env), 0)
            self.assertEqual(dry_wp.created, []); self.assertEqual(dry_wp.updated, [])
            saved = json.loads((Path(td) / "planned.json").read_text())
            self.assertEqual(saved["schema_version"], 2)
            self.assertEqual(saved["write_policy"], schema_mod.WRITE_FILL_ONLY)
            ...
            original_materialize = plans_mod.materialize_body
            with patch.object(cli_mod, "WordPress", return_value=run_wp), \
                 patch.object(cli_mod, "Spotify"), patch.object(cli_mod, "LastFM"), \
                 patch.object(cli_mod, "enrich", return_value=patch_row()), \
                 patch.object(cli_mod, "materialize_body", wraps=original_materialize) as materialize:
                self.assertEqual(cli_mod.cmd_run(args, env), 0)
                self.assertEqual(materialize.call_count, 1)
            apply_args = Namespace(plan=str(Path(td) / "planned.json"), offset=0,
                                   limit=None, out_dir=td)
            with patch.object(cli_mod, "WordPress", return_value=replay_wp), \
                 patch.object(cli_mod, "materialize_body", wraps=original_materialize) as materialize:
                self.assertEqual(cli_mod.cmd_apply_plan(apply_args, env), 0)
                self.assertEqual(materialize.call_count, 1)
    ...
    def test_run_continues_after_malformed_spotify_track_and_validates_plan(self):
        ...
        try:
            spotify_mod.validate_spotify_album_tracks(album, [track])
        except providers_mod.SpotifyProviderError as exc:
            malformed = enrichment_mod._provider_unresolved(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(cli_mod, "WordPress", return_value=CommandWP()), \
                 patch.object(cli_mod, "Spotify"), patch.object(cli_mod, "LastFM"), \
                 patch.object(cli_mod, "enrich", side_effect=[malformed, patch_row(2)]):
                self.assertEqual(cli_mod.cmd_run(args, env), 0)
            planned = json.loads((Path(td) / "planned.json").read_text())
            unresolved = json.loads((Path(td) / "unresolved.json").read_text())
            plans_mod.validate_plan(planned)
            plans_mod.validate_unresolved(unresolved)
    ...
    def test_outage_writes_artifacts_exits_nonzero_and_blocks_deprecated_apply(self):
        ...
        class Client:
            def __init__(self, provider): self._circuit = providers_mod.ProviderCircuit(provider)
        ...
        for apply in (False, True):
            with self.subTest(apply=apply), tempfile.TemporaryDirectory() as td:
                ...
                def failed(post, *args, **kwargs):
                    ...
                    error = providers_mod.SpotifyProviderError(
                    ...
                    return enrichment_mod._provider_unresolved(post, "spotify_provider_error", error,
                                                    "album.search")
                args = Namespace(offset=0, limit=None, out_dir=td, apply=apply,
                                 dry_run=not apply, overwrite_managed=False)
                with patch.object(cli_mod, "WordPress", return_value=wp), \
                     patch.object(cli_mod, "Spotify", return_value=spotify), \
                     patch.object(cli_mod, "LastFM", return_value=lastfm), \
                     patch.object(cli_mod, "enrich", side_effect=failed), \
                     patch.object(cli_mod, "apply_patches") as apply_patches:
                    self.assertEqual(cli_mod.cmd_run(args, env), 1)
    ...
    def test_cmd_run_propagates_policy_and_loads_release_types_once(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.object(cli_mod, "WordPress", return_value=wp), \
                 patch.object(cli_mod, "Spotify"), patch.object(cli_mod, "LastFM"), \
                 patch.object(cli_mod, "enrich", return_value=None) as enrich, \
            ...
            self.assertTrue(all(call.args[-1] == schema_mod.WRITE_OVERWRITE_MANAGED
                                for call in enrich.call_args_list))
            self.assertTrue(all(call.kwargs["release_type_terms"] == {"Album": 30}
                                for call in enrich.call_args_list))
            saved = json.loads((Path(td) / "planned.json").read_text())
            self.assertEqual(saved["write_policy"], schema_mod.WRITE_OVERWRITE_MANAGED)
            output = "\n".join(logs.output)
    ...
    def test_atomic_writer_replaces_complete_json(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            common_mod.write_json_atomic(path, {"ok": True})
